...
def create_corpus(corpus_id):
    return requests.post("https://api.vectara.io/v1/create-corpus",
                         headers={"Content-Type": "application/json", "Accept": "application/json",
                                  "customer-id": "2698881212",
                                  "Authorization": "Bearer " + vectara_config["bearer-key"]},
                         data=json.dumps({"corpus": {"id": corpus_id}}))


def list_corpus():
    return requests.post("https://api.vectara.io/v1/list-corpora",
                         headers={"Content-Type": "application/json", "Accept": "application/json",
                                  "customer-id": "2698881212",
                                  "Authorization": "Bearer " + vectara_config["bearer-key"]},
                         data=json.dumps({"numResults": 999}))


def upload_file_to_corpus(corpus_id, file):
    return requests.post("https://api.vectara.io/v1/upload",
                         headers={"Content-Type": "multipart/form-data", "Accept": "application/json",
                                  "x-api-key": vectara_config["api-key"]}, data={}, files={"file": file},
                         params={"c": "2698881212", "o": corpus_id})


def query_corpus(corpus_id, text):
    response = requests.post("https://api.vectara.io/v1/query",
                             headers={"Content-Type": "application/json", "Accept": "application/json",
                                      "customer-id": "2698881212", "x-api-key": vectara_config["api-key"]},
                             data=json.dumps({"query": [{"query": text, "numResults": 5, "corpusKey": [
                                 {"customerId": 2698881212, "corpusId": corpus_id}]}]}))
    relevant_text = []
...
def convert_to_pdf(filename):
    ...
    return output.get("result").get("files")[0]["filename"], requests.get(
        output.get("result").get("files")[0]["url"]).content


@app.route("/substitute")
async def substitute():
    file = (await request.files).pop(
        "slideshow")  # rest of the files, past the slideshow, will be google classroom documents
    ...
    for file in (
    await request.files).values():  # rest of the files, past the slideshow, will be google classroom documents
...
async def attendance():
    ...
    return jsonify(
        {"present": list(closest_texts.keys()), "absent": list(set(text_bbox_dict.keys()) - set(closest_texts.keys()))})
...
async def generate_curriculum():
    ...
    response = requests.post("https://api.openai.com/v1/chat/completions",
                             ...
                             json={"model": "gpt-4", "messages": [
                                 {"role": "system", "content": [{"type": "text", "text": system_message}]},
                                 {"role": "user", "content": [{"type": "text", "text": prompt}]}], "max_tokens": 1500})
